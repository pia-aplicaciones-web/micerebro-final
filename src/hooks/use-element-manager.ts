  ...
  const addElement = useCallback(async (type: ElementType, props?: {
    ...
    switch (type) {
      ...
      case 'mini':
        ...
        break;
      case 'dictado':
        // Tamaño fijo: formato vertical (256px x 600px - ancho reducido 20% adicional)
        const dictadoSize = { width: 256, height: 600 };
      ...
      case 'pomodoro-timer':
        ...
        newElementData = { type, x: pomodoroPos.x, y: pomodoroPos.y, width: pomodoroSize.width, height: pomodoroSize.height, userId, properties: { ...baseProperties, position: pomodoroPos, size: pomodoroSize, backgroundColor: '#ffffff' }, content: { durationSeconds: 25 * 60, remainingSeconds: 25 * 60, running: false }, zIndex, createdAt: serverTimestamp(), updatedAt: serverTimestamp() } as Omit<WithId<CanvasElement>, 'id'> & { type: ElementType }; break;
      case 'gallery':
