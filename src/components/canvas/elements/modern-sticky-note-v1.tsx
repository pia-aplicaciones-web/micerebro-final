...
/**
 * PROPUESTA 1: Glassmorphism Elegante
 * Efecto de vidrio esmerilado con blur, sombras suaves y diseño minimalista
 */
...
import { 
  ...
  Maximize, Copy, Camera, FileImage, RotateCw 
} from 'lucide-react';
...
const TEXT_COLOR = '#4A5568'; // Gris oscuro medio

export default function ModernStickyNoteV1(props: CommonElementProps) {
  const {
    id,
    content,
    properties,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundColor: `${currentColor.bg}CC`, // 80% opacity
          boxShadow: `0 8px 32px 0 rgba(0,0,0,0.1), inset 0 1px 0 0 rgba(255,255,255,0.5)`,
        }}
      >
        {/* Floating Header - Glassmorphism */}
        <div className={cn(
          "absolute top-3 left-3 right-3 flex items-center gap-1.5 z-20 transition-opacity duration-200",
          "bg-white/60 backdrop-blur-md rounded-xl px-2 py-1.5 shadow-lg border border-white/50",
          isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"
        )}>
          <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-white/40" onClick={handleClose} onMouseDown={(e) => e.stopPropagation()}>
            <X className="h-3.5 w-3.5" />
          </Button>
          <div className="drag-handle cursor-grab active:cursor-grabbing p-1 hover:bg-white/40 rounded">
            <GripVertical className="h-3.5 w-3.5" />
          </div>
          <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-white/40" onClick={toggleMinimize} onMouseDown={(e) => e.stopPropagation()}>
            <Minus className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-white/40" onClick={handleForceSave} onMouseDown={(e) => e.stopPropagation()}>
            <Save className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className={cn("h-7 w-7 hover:bg-white/40", isReading && "bg-blue-100/60")} onClick={handleReadAloud} onMouseDown={(e) => e.stopPropagation()}>
            <Volume2 className={cn("h-3.5 w-3.5", isReading && "text-blue-600")} />
          </Button>
          {isSelected && (
            <>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-white/40" onClick={(e) => e.stopPropagation()}>
                    <Paintbrush className="h-3.5 w-3.5" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-3 bg-white/90 backdrop-blur-md border-white/50 shadow-xl">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-white/40">
                    <Maximize className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="bg-white/90 backdrop-blur-md border-white/50">
