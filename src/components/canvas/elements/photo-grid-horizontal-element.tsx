...
export default function PhotoGridHorizontalElement(props: CommonElementProps) {
  ...
  const gridContent: PhotoGridContent = isPhotoGridContent(content)
    ? content
    : { title: 'Guía de Fotos Horizontal', rows: 2, columns: 2, cells: [], layoutMode: 'horizontal' };
  ...
  const handleExportPng = useCallback(async () => {
    ...
    try {
      ...
      link.download = `${gridContent.title || 'guia-fotos-horizontal'}-${Date.now()}.png`;
