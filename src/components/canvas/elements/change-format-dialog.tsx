...
interface ChangeFormatDialogProps {
  ...
  onSaveFormat: (id: string, format: 'letter' | '10x15' | '20x15') => void;
}
...
const formats = [
  ...
  { id: '20x15', label: 'Apuntes (20cm x 15cm)', description: 'Formato horizontal para apuntes extensos.' },
];
...
export default function ChangeFormatDialog({
  ...
  const [selectedFormat, setSelectedFormat] = useState<'letter' | '10x15' | '20x15'>('letter');

  useEffect(() => {
    if (notepad) {
      const currentFormat = (notepad.properties as NotepadElementProperties)?.format || 'letter';
      setSelectedFormat(currentFormat as 'letter' | '10x15' | '20x15');
    }
  }, [notepad]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="py-4">
          <RadioGroup value={selectedFormat} onValueChange={(value) => setSelectedFormat(value as 'letter' | '10x15' | '20x15')}>
            {formats.map((format) => (
              <Label
                key={format.id}
                htmlFor={format.id}
