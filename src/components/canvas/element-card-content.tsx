...
const ElementCardContent: React.FC<ElementCardContentProps> = ({
  ...
  const renderPreview = () => {
    switch (element.type) {
      ...
      case 'notepad':
        const notepadContent = element.content as NotepadContent;
        const pages = notepadContent.pages || [];
        const currentPage = typeof notepadContent.currentPage === 'number' ? notepadContent.currentPage : 0;
        const pageContent = pages && pages.length > 0 && currentPage < pages.length ? pages[currentPage] : '';
